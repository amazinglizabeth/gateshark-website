    ...
    <section id="features" className="py-20 px-6 bg-slate-50">
      <div className="max-w-300 mx-auto grid grid-cols-1 md:grid-cols-3 gap-12">
        <div className="space-y-4">
          <h2 className="text-3xl md:text-4xl font-medium text-[57px] text-[#132C25] leading-tight">
        ...
        <div className="md:col-span-2 grid grid-cols-1 sm:grid-cols-2 gap-6">
          {featuresList.map((item, idx) => (
            <div key={idx} className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm space-y-3">
              ...
              <h3 className="font-inter font-medium text-[18px] text-[#132C24]">{item.title}</h3>
              <p className="font-inter text-[#6D6D6D] text-[14.8px] font-[regular] leading-relaxed">{item.desc}</p>
            </div>
          ))}
        </div>
      </div>
